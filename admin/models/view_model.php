...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    if($qry->num_rows > 0){
		...
		echo '<script>alert("معرف الطراز غير صالح."); location.replace("./?page=models")</script>';
	}
}else{
	echo '<script>alert("معرف النموذج مطلوب."); location.replace("./?page=models")</script>';
}
?>
<style>
	#model-thumbnail{
...
<div class="content py-5 px-3 bg-gradient-navy">
	<h4 class="font-wight-bolder">تفاصيل</h4>
</div>
<div class="row mt-n4 align-items-center justify-content-center flex-column">
	<div class="col-lg-10 col-md-11 col-sm-12 col-xs-12">
		<div class="card rounded-0 shadow">
			<div class="card-footer py-1 text-center">
				<button class="btn btn-flat btn-sm btn-primary bg-gradient-primary" type="button" id="add_vehicle"><i class="far fa-plus-square"></i> اضافة مركبة</button>
				<a class="btn btn-flat btn-sm btn-navy bg-gradient-navy border" href="./?page=models/manage_model&id=<?= isset($id) ? $id : '' ?>"><i class="fa fa-edit"></i> تحرير</a>
				<button class="btn btn-flat btn-sm btn-danger bg-gradient-danger" type="button" id="delete_model"><i class="fa fa-trash"></i> حذف </button>
				<a class="btn btn-flat btn-sm btn-light bg-gradient-light border" href="./?page=models"><i class="fa fa-angle-left"></i> الرجوع</a>
			</div>
		</div>
		<div class="card rounded-0 shadow">
			<div class="card-body">
				<div class="container-fluid">
					...
					<div class="d-flex w-100">
						<div class="col-3 mb-0 border bg-gradient-secondary">موديل</div>
					...
					<div class="d-flex w-100 mb-3">
						<div class="col-3 mb-0 border bg-gradient-secondary">الحالة:</div>
				...
				<div class="container-fluid">
					<table class="table table-stripped table-bordered" id="vehicle-list">
						...
						<thead>
							...
							<th class="p-1 text-center">رقم السيارة </th>
							<th class="p-1 text-center">الامتار</th>
							<th class="p-1 text-center">السعر</th>
							<th class="p-1 text-center">الحالة</th>
							<th class="p-1 text-center">تحرير</th>
						</thead>
						<tbody>
							<?php if(isset($id)): ?>
							<?php 
							...
							<tr>
								...
								<td class="p-1 align-middle"><?= $row['mileage'] ?> KM</td>
								...
								<td class="p-1 align-middle text-center">
									...
									<?php else: ?>
										<span class="badge badge-danger px-3 rounded-pill">تم البيع</span>
									<?php endif; ?>
								</td>
								<td class="p-1 align-middle text-center">
									<button class="btn btn-flat btn-light bg-gradient-light btn-sm border view_vehicle" data-id="<?= $row['id'] ?>" type="button"><i class="fa fa-eye"></i> عرض</button>
								</td>
							</tr>
							<?php endwhile; ?>
							<?php endif; ?>
...
<script>
	$(function(){
		...
		$('#add_vehicle').click(function(){
			uni_modal('<i class="far fa-plus-square"></i> اضافة مركبة جديدة','models/manage_vehicle.php?model_id=<?= isset($id) ? $id : '' ?>','modal-lg')
		})
		$('.view_vehicle').click(function(){
			uni_modal('<i class="fa fa-edit"></i> تفاصيل السيارة','models/view_vehicle.php?id='+$(this).attr('data-id'))
		})
		...
		$.ajax({
			...
			success:function(resp){
				if(typeof resp== 'object' && resp.status == 'success'){
					...
					img.remove()
					alert_toast('تم حذف الصورة بنجاح.','success')
					$('.modal').modal('hide')
				}else{
					alert_toast("An error occured.",'error');
				}
