...
<div class="col-lg-12">
	<div class="card card-outline rounded-0 card-navy">
		...
		<div class="card-body">
			<form action="" id="system-frm">
				...
				<div class="form-group">
					<label for="name" class="control-label">اسم النظام</label>
					<input type="text" class="form-control form-control-sm" name="name" id="name" value="<?php echo $_settings->info('name') ?>">
				</div>
				<div class="form-group">
					<label for="short_name" class="control-label">الاسم المختصر</label>
			...
			<div class="form-group">
				<label for="" class="control-label">صورة الواجهة</label>
				<div class="custom-file">
	              ...
	              <label class="custom-file-label" for="customFile3">تحميل</label>
	            </div>
				<small><i>اختر تحميل صورة جديدة</i></small>
			</div>
			<?php 
		...
		<div class="card-footer">
			<div class="col-md-12">
				<div class="row">
					<button class="btn btn-sm btn-primary" form="system-frm">تحديث</button>
				</div>
			</div>
		</div>
...
<script>
	...
	function delete_img($path){
        ...
        $.ajax({
            ...
            error:err=>{
                console.log(err)
                alert_toast("حدث خطأ أثناء حذف الصورة","error");
                end_loader()
            },
	...
	$(document).ready(function(){
		$('.rem_img').click(function(){
            _conf("هل  انت متاكد من حذف هذا",'delete_img',["'"+$(this).attr('data-path')+"'"])
        })
